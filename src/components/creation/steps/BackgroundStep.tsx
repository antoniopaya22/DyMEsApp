...
export default function BackgroundStep() {
  ...
  const { draft, setTrasfondo, setCustomBackgroundData, saveDraft, loadDraft } =
    useCreationStore();

  const [selectedBg, setSelectedBg] = useState<BackgroundId | null>(null);
  const [showDetails, setShowDetails] = useState(false);
  const [customData, setCustomData] = useState<
    CustomBackgroundConfig | undefined
  >(undefined);
  const [showExpansions, setShowExpansions] = useState(
    draft?.trasfondo
      ? EXPANSION_BACKGROUND_IDS.includes(draft.trasfondo)
      : false,
  );

  const allBackgrounds = getBackgroundList();
  const backgrounds = allBackgrounds.filter(
    (bg) => !EXPANSION_BACKGROUND_IDS.includes(bg.id),
  );
  const expansionBackgrounds = allBackgrounds.filter((bg) =>
    EXPANSION_BACKGROUND_IDS.includes(bg.id),
  );
  ...
  const isCustomValid =
    selectedBg === "personalizada" &&
    customData &&
    customData.nombre.trim().length >= 1;
                ...
                return (
                  <View key={bg.id}>
                    <SelectionCard
                      iconName={icon}
                      title={bg.nombre}
                      subtitle={bg.skillProficiencies
                        .map(formatSkillName)
                        .join(", ")}
                      isSelected={isSelected}
                      onPress={() => handleSelectBg(bg.id)}
