...
import SelectionCard, {
  ExpansionToggleCard,
  PillBadge,
} from "@/components/creation/SelectionCard";
  ...
  return (
    <>
      {Object.entries(race.abilityBonuses).map(([key, value]) => (
        <PillBadge
          key={key}
          text={`${ABILITY_NAMES[key as AbilityKey]} ${formatBonus(value as number)}`}
          color={colors.accentGold}
        />
      ))}
      {race.freeAbilityBonusCount ? (
        <PillBadge
          text={`+${race.freeAbilityBonusCount} a elegir`}
          color={colors.accentGold}
        />
      ) : null}
      {race.flySpeed ? (
        <PillBadge text="Vuelo" icon="airplane-outline" />
      ) : null}
      {race.darkvision ? (
        <PillBadge text="Visión osc." icon="eye-outline" />
      ) : null}
    </>
  );
}
...
export default function RaceSelectionStep() {
  ...
  const [modalLineage, setModalLineage] = useState<string | undefined>(
    draft?.dragonLineage,
  );
  const [modalToolChoice, setModalToolChoice] = useState<string | undefined>(
    draft?.raceToolChoice,
  );
  ...
  const expansionRaces = allRaces.filter((r) =>
    EXPANSION_RACE_IDS.includes(r.id),
  );
  ...
  const detailRaceData =
    detailRaceId && detailRaceId !== "personalizada"
          ...
          <View className="px-5">
            ...
            >
              <View className="flex-row items-center">
                ...
                >
                  <Ionicons
                    name={RACE_ICONS["personalizada"] as any}
                    size={32}
          ...
          >
                ...
                <>
                  ...
                  >
                    <View className="flex-row items-center mb-4">
                      ...
                      >
                        <Ionicons
                          name={RACE_ICONS[detailRaceData.id] as any}
                          size={32}
                          color={colors.accentRed}
                        />
                      </View>
                      <View className="flex-1">
                        ...
                        >
                          Tamaño:{" "}
                          {detailRaceData.size === "mediano"
                            ? "Mediano"
                            : "Pequeño"}{" "}
                          · Velocidad:{" "}
                          {formatDistancia(detailRaceData.speed, unidades)}
                        </Text>
                      </View>
                    </View>
                      ...
                      <View className="flex-row items-center">
                        ...
                        <View className="flex-1">
                          ...
                          >
                            Alcance:{" "}
                            {formatDistancia(
                              detailRaceData.darkvisionRange ?? 60,
                              unidades,
                            )}
                          </Text>
                        </View>
                  ...
                  {detailRaceData.weaponProficiencies &&
                    detailRaceData.weaponProficiencies.length > 0 && (
                      <View className="mb-5">
                        ...
                        <View className="flex-row flex-wrap">
                          {detailRaceData.weaponProficiencies.map(
                            (weapon, idx) => (
                              <View
                                key={idx}
                                className="border rounded-full px-3 py-1.5 mr-2 mb-2"
                  ...
                  {detailRaceData.toolChoices &&
                    detailRaceData.toolChoices.length > 0 && (
                      <View className="mb-5">
                        ...
                        >
                          Herramientas (elige{" "}
                          {detailRaceData.toolChoiceCount ?? 1})
                        </Text>
                        <View className="flex-row flex-wrap">
                  ...
                  {detailRaceData.skillProficiencies &&
                    detailRaceData.skillProficiencies.length > 0 && (
                      <View className="mb-5">
                        ...
                        <View className="flex-row flex-wrap">
                          {detailRaceData.skillProficiencies.map(
                            (skill, idx) => {
            ...
            >
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: isModalSelectable()
                    ? colors.accentRed
                    : colors.bgSecondary,
