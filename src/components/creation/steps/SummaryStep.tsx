...
// (creation/index.ts re-exports from ./steps, which exports this file)
...
export default function SummaryStep() {
  ...
  const { campaignId, isCampaignMode, pushStep, goBack, replaceAfterCreation } =
    useCreationNavigation();
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          if (!isRecreation) {
            if (isCampaignMode) {
              // Campaign mode: create default empty inventory
              ...
              // Standalone mode: build full starting inventory from class + background equipment
              const { buildStartingInventory } =
                await import("@/utils/inventory");
              const { getClassData: getClass, getBackgroundData: getBg } =
                await import("@/data/srd");
              const { buildBackgroundDataFromCustom } =
                await import("@/data/srd/backgrounds");

              const currentDraft = useCreationStore.getState().draft;
              const cls = currentDraft?.clase
                ? getClass(currentDraft.clase)
                : null;
              const bg =
                currentDraft?.trasfondo === "personalizada" &&
                currentDraft?.customBackgroundData
                  ? buildBackgroundDataFromCustom(
                      currentDraft.customBackgroundData,
                    )
                  : currentDraft?.trasfondo
                    ? getBg(currentDraft.trasfondo)
                    : null;

              const inventory = buildStartingInventory({
                inventoryId: character.inventoryId,
                characterId: character.id,
                classEquipmentChoices: cls?.equipmentChoices ?? [],
                selectedChoices: currentDraft?.equipmentChoices ?? {},
                classDefaultEquipment: cls?.defaultEquipment ?? [],
                backgroundEquipment: bg?.equipment ?? [],
                backgroundStartingGold: bg?.startingGold ?? 0,
              });
              await setItem(STORAGE_KEYS.INVENTORY(character.id), inventory);
              console.log(
                "[SummaryStep] Inventario creado con",
      ...
      {
        confirmText: isRecreation ? "¡Recrear!" : "¡Crear!",
        cancelText: "Revisar",
        type: "confirm",
      },
    );
  ...
  const handleGoToStep = (step: number) => {
    const route = STEP_ROUTES[step] as WizardStep | undefined;
    if (route) {
      pushStep(route);
    }
  };
