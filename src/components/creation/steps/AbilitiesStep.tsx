...
import { ABILITY_KEYS, getAbilityColors } from "@/constants/abilities";
import { STANDARD_ARRAY, POINT_BUY_COSTS, POINT_BUY_TOTAL } from "@/utils/dice";
...
import { useCompraPuntos } from "@/stores/settingsStore";
...
export default function AbilitiesStep() {
  ...
  const compraPuntos = useCompraPuntos();
  ...
  ).filter((m) => m !== "point_buy" || compraPuntos);

  const {
    draft,
    setAbilityScoreMethod,
    setAbilityScores,
    setFreeAbilityBonuses,
    saveDraft,
    loadDraft,
  } = useCreationStore();
  ...
  const [mAssigned, setMAssigned] = useState<Record<AbilityKey, number | null>>(
    { ...NULL_ASSIGNED },
  );
  const [mHasRolled, setMHasRolled] = useState(false);

  // Free racial bonus state
  const [freeBonuses, setFreeBonuses] = useState<AbilityKey[]>([]);

  // Derived: race free bonus info
  const raceData =
    draft?.raza && draft.raza !== "personalizada"
      ? getRaceData(draft.raza)
      : null;
  const freeCount = raceData?.freeAbilityBonusCount ?? 0;
  // Keys that already have a fixed racial bonus — exclude from free picks
  const fixedBonusKeys = new Set(
    Object.keys(raceData?.abilityBonuses ?? {}).filter(
      (k) => (raceData?.abilityBonuses[k as AbilityKey] ?? 0) > 0,
    ),
  );
  const freePickableKeys = ABILITY_KEYS.filter((k) => !fixedBonusKeys.has(k));
  const freeComplete = freeCount === 0 || freeBonuses.length === freeCount;
  ...
  useFocusEffect(
    useCallback(() => {
      const init = async () => {
        ...
        if (currentDraft?.abilityScoresBase) {
          ...
          if (currentDraft.abilityScoreMethod === "standard_array") {
            ...
            for (const key of ABILITY_KEYS) {
              ...
              if ((STANDARD_ARRAY as readonly number[]).includes(val)) {
                assigned[key] = val;
              }
            }
        ...
        if (currentDraft?.freeAbilityBonuses) {
          setFreeBonuses(currentDraft.freeAbilityBonuses);
        }
      };
      init();
    }, [campaignId]),
  ...
  const handleNext = async () => {
    ...
    if (freeCount > 0) setFreeAbilityBonuses(freeBonuses);
    await saveDraft();
    pushStep("background");
  };
  ...
  const totalScores = draft?.raza
    ? calcTotalScoresPreview(
        ...
        draft.raza,
        draft.subraza ?? null,
        freeBonuses.length > 0 ? freeBonuses : draft.freeAbilityBonuses,
  ...
  return (
    <>
      <WizardStepLayout
        ...
        canProceed={!!method && freeComplete}
        onNext={handleNext}
        onBack={handleBack}
        scrollRef={scrollRef}
      >
        ...
        <View style={styles.section}>
          ...
          {availableMethods.map((m) => (
            ...
            >
              ...
              <Ionicons
                name={method === m ? "checkmark-circle" : "chevron-forward"}
                size={22}
                color={method === m ? colors.accentGold : colors.textMuted}
              />
            </TouchableOpacity>
        ...
        {method && (
          <View style={styles.section}>
                ...
                {ABILITY_KEYS.map((key) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {racial !== 0 && (
                        <Text style={[styles.previewRacial, themed.textMuted]}>
                          {base} + {racial}
                        </Text>
                      )}
                    </View>
            ...
            >
              ...
              />
              <Text
                style={[styles.editScoresBtnText, { color: colors.accentRed }]}
              >
                Editar puntuaciones
              </Text>
            </TouchableOpacity>
        ...
        {/* Free Racial Bonus Picker */}
        {method && freeCount > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, themed.sectionTitle]}>
              Bonificadores Raciales (+1)
            </Text>
            <View style={[styles.previewSection, themed.card]}>
              {/* Header with counter */}
              <View
              ...
              >
                ...
                >
                  ...
                  >
                    Elige {freeCount} característica{freeCount > 1 ? "s" : ""}
                  </Text>
                </View>
                <View
                  style={{
                ...
                >
                  <Ionicons
                    name={
                      freeComplete ? "checkmark-circle" : "ellipsis-horizontal"
                    }
                    size={16}
                    color={freeComplete ? colors.accentGreen : colors.textMuted}
                  />
                  <Text
                    style={{
                      color: freeComplete
              ...
              >
                {freePickableKeys.map((key) => {
                  const selected = freeBonuses.includes(key);
                  const abilityColors = getAbilityColors(colors);
                  const abilityColor = abilityColors[key];
                  const canSelect = !selected && freeBonuses.length < freeCount;
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        backgroundColor: selected
                          ? withAlpha(abilityColor, 0.15)
                          : colors.bgSecondary,
                        borderColor: selected
                          ? withAlpha(abilityColor, 0.5)
                    ...
                    >
                      ...
                      <Text
                        style={{
                          color: selected ? abilityColor : colors.textSecondary,
                          fontSize: 14,
                          fontWeight: "700",
                        }}
                      >
              ...
              {/* Fixed bonuses info (e.g., semielfo CAR +2) */}
              {fixedBonusKeys.size > 0 && (
                <View
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                ...
                >
                  ...
                  >
                    Fijos:{" "}
                    {Array.from(fixedBonusKeys)
                      .map(
                        (k) =>
                          `${ABILITY_ABBR[k as AbilityKey]} +${raceData?.abilityBonuses[k as AbilityKey]}`,
                      )
                      .join(", ")}
                  </Text>
                </View>
                        ...
                        mAssigned[key] !== null ? (
                          ...
                          >
                            <View style={styles.abilityLabel}>
                              ...
                              <Text
                                style={[styles.abilityName, themed.textPrimary]}
                              >
                                {ABILITY_NAMES[key]}
                              </Text>
                            </View>
                          ...
                          <View
                            key={key}
                            style={[styles.abilityRowVertical, themed.card]}
                          >
                            <View style={styles.abilityLabel}>
                              <Text
                                style={[styles.abilityAbbr, themed.textAccent]}
                          ...
                          <View style={styles.spinnerRow}>
                            <TouchableOpacity
                              style={[
                                styles.spinnerBtn,
                                themed.spinnerBtn,
                                mScores[key] <= 8 && styles.spinnerBtnDisabled,
                              ]}
                              onPress={() => handleModalPointBuyChange(key, -1)}
                            ...
                            <Text
                              style={[styles.spinnerValue, themed.textPrimary]}
                            >
                              {mScores[key]}
                            </Text>
                            <TouchableOpacity
                              style={[
                                styles.spinnerBtn,
                                themed.spinnerBtn,
                                mScores[key] >= 15 && styles.spinnerBtnDisabled,
                              ]}
                              onPress={() => handleModalPointBuyChange(key, 1)}
                  ...
                  {modalMethod === "dice_roll" && (
                    <View>
                      ...
                      >
                        ...
                        />
                        <Text
                          style={[styles.rollButtonText, themed.textOnPrimary]}
                        >
                      ...
                      {mHasRolled &&
                        ABILITY_KEYS.map((key) => (
                          ...
                          >
                            ...
                            <Text
                              style={[styles.rolledValue, themed.textPrimary]}
                            >
                              {mScores[key]}
                            </Text>
                          </View>
                          ...
                          <View style={styles.spinnerRow}>
                            <TouchableOpacity
                              style={[
                                styles.spinnerBtn,
                                themed.spinnerBtn,
                                mScores[key] <= 1 && styles.spinnerBtnDisabled,
                              ]}
                              onPress={() => handleModalManualChange(key, -1)}
                            ...
                            <TouchableOpacity
                              style={[
                                styles.spinnerBtn,
                                themed.spinnerBtn,
                                mScores[key] >= 30 && styles.spinnerBtnDisabled,
                              ]}
                              onPress={() => handleModalManualChange(key, 1)}
                      ...
                      <View style={styles.previewGrid}>
                        {ABILITY_KEYS.map((key) => {
                          ...
                          return (
                            ...
                            >
                              <Text
                                style={[styles.previewAbbr, themed.textAccent]}
                              >
                                {ABILITY_ABBR[key]}
                              </Text>
                              <Text
